"""Convert sops/cbrn.md to sops/10.16.2-sop-cbrn.html (authoritative markdown source)."""
...
MD_PATH = ROOT / "cbrn.md"
OUT_FRAG = ROOT / "_cbrn_body.frag.html"
OUT_PAGE = ROOT / "10.16.2-sop-cbrn.html"
...
<p>Commanding</p>
<hr>
<p><strong>CMDP Reference:</strong> 10(16)-2</p>
"""
...
<head>
    ...
    <title>CBRN EQUIPMENT MAINTENANCE STANDARD OPERATING PROCEDURE - CMDP Compliance</title>
...
def strip_banner_and_cover(lines: list[str]) -> list[str]:
    ...
    while i < len(lines) and not lines[i].strip():
        i += 1
    if i < len(lines) and lines[i].startswith("# CBRN EQUIPMENT"):
        i += 1
    while i < len(lines) and not lines[i].strip():
        i += 1
...
def main() -> None:
    ...
    header_html = f"""<hr>
<h1>CBRN EQUIPMENT MAINTENANCE STANDARD OPERATING PROCEDURE</h1>
