"""Convert sops/arms-room.md to sops/10.15.1-sop-arms-room.html (authoritative markdown source)."""
...
MD_PATH = ROOT / "arms-room.md"
OUT_FRAG = ROOT / "_arms_room_body.frag.html"
OUT_PAGE = ROOT / "10.15.1-sop-arms-room.html"
...
<p>Commanding</p>
<hr>
<p><strong>CMDP Reference:</strong> 10(15)-1</p>
"""
...
<head>
    ...
    <title>ARMS ROOM AND WEAPONS MAINTENANCE STANDARD OPERATING PROCEDURE - CMDP Compliance</title>
...
def strip_banner_and_cover(lines: list[str]) -> list[str]:
    ...
    while i < len(lines) and not lines[i].strip():
        i += 1
    if i < len(lines) and lines[i].startswith("# ARMS ROOM"):
        i += 1
    while i < len(lines) and not lines[i].strip():
        i += 1
...
def main() -> None:
    ...
    header_html = f"""<hr>
<h1>ARMS ROOM AND WEAPONS MAINTENANCE STANDARD OPERATING PROCEDURE</h1>
