...
def _split_md_table_row_pipe(line: str) -> list[str]:
    """Split a markdown pipe row into cells (allows missing trailing '|')."""
    raw = line.strip()
...
def _resolve_image_path_for_md(md_source: Path, src: str) -> Path | None:
    """Resolve ![](src) for SOP Markdown (e.g. ../memos/img → repo memos/img)."""
    raw = src.strip()
    cand = (md_source.parent / raw).resolve()
    if cand.is_file():
        return cand
    stripped = raw
    while stripped.startswith("../"):
        stripped = stripped[3:]
        trial = (md_source.parent / stripped).resolve()
        if trial.is_file():
            return trial
        trial2 = repo_root() / stripped
        if trial2.is_file():
            return trial2
    trial3 = repo_root() / stripped.lstrip("./")
    if trial3.is_file():
        return trial3
    by_name = repo_root() / "memos" / "img" / Path(raw).name
    if by_name.is_file():
        return by_name
    return None
...
def convert_markdown_table_to_word(doc, table_lines):
    """Convert markdown pipe table to Word table (AR 25–50 body text in cells)."""
    rows: list[list[str]] = []
    for raw_ln in table_lines:
        parts = _split_md_table_row_pipe(raw_ln)
        if not parts:
            continue
        if all(not c or re.fullmatch(r"[\s\-:]+", c) for c in parts):
            continue
        rows.append(parts)
...
def convert_markdown_to_docx(md_file_path, output_dir):
    ...
    md_fp = Path(md_file_path).resolve()
    ...
    while i < len(lines):
        ...
        # Markdown images → inline picture when file exists locally
        m_img = re.fullmatch(r"!\[([^\]]*)\]\(\s*(.+?)\s*\)", line.strip())
        if m_img:
            ip = _resolve_image_path_for_md(md_fp, m_img.group(2))
            if ip and ip.is_file():
                pic_para = doc.add_paragraph()
                pic_para.alignment = WD_ALIGN_PARAGRAPH.LEFT
                pic_para.paragraph_format.space_before = Pt(0)
                pic_para.paragraph_format.space_after = Pt(12)
                run_pic = pic_para.add_run()
                run_pic.add_picture(str(ip), width=Inches(2.3))
            else:
                create_paragraph_with_formatting(
                    doc, f"[Image not found: {m_img.group(2).strip()}]", is_empty=False
                )
            i += 1
            continue
        ...
        # Non-numbered section headings (e.g. Administrative Data), before numbered ## sections
        if line.startswith('## ') and not re.match(r'^##\s+\d+\.', line):
            heading_text = re.sub(r'^##\s+', '', line).strip()
            clean_text = re.sub(r'\*\*([^*]+)\*\*', r'\1', heading_text)
            create_paragraph_with_formatting(doc, clean_text, bold=True, is_heading=True)
            i += 1
            continue
...
def main():
    ...
    _root = repo_root()
    sops_dir = _root / 'source' / 'sops'
    output_dir = _root / 'assets' / 'word' / 'sops'
    ...
    print(
        "Local markdown images (![](path)) were embedded when paths resolve relative to repo."
    )
