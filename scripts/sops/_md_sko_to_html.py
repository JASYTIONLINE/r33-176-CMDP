"""Convert sops/sko.md to sops/10.11.1-sop-tool-room.html (authoritative markdown source)."""
...
MD_PATH = _MD / "sko.md"
OUT_FRAG = _PUB / "_sko_body.frag.html"
OUT_PAGE = _PUB / "10.11.1-sop-tool-room.html"
...
<p>Commanding</p>
<hr>
<p><strong>CMDP Reference:</strong> 10(11)-1</p>
"""
...
<head>
    ...
    <title>TOOL ROOM AND SKOT MAINTENANCE STANDARD OPERATING PROCEDURE - CMDP Compliance</title>
...
def strip_banner_and_cover(lines: list[str]) -> list[str]:
    ...
    while i < len(lines) and not lines[i].strip():
        i += 1
    if i < len(lines) and lines[i].startswith("# TOOL ROOM AND SKOT"):
        i += 1
    while i < len(lines) and not lines[i].strip():
        i += 1
...
def main() -> None:
    ...
    header_html = f"""<hr>
<h1>TOOL ROOM AND SKOT MAINTENANCE STANDARD OPERATING PROCEDURE</h1>
