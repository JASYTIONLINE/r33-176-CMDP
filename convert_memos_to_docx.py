...
def main():
    """Main function to convert all memo markdown files"""
    memos_dir = Path('content/memos')
    output_dir = Path('content/word')
    
    # Files to exclude
    exclude_files = {'memos.index.md', '00.0-example-memo.md'}
    
    # Find all .md files
    md_files = [f for f in memos_dir.glob('*.md') 
                if f.name not in exclude_files]
    
    print(f"Found {len(md_files)} memo files to convert")
