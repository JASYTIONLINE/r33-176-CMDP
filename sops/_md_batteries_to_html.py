"""Convert sops/batteries.md to sops/10.14.3-sop-battery-program.html (authoritative markdown source)."""
...
MD_PATH = ROOT / "batteries.md"
OUT_FRAG = ROOT / "_battery_body.frag.html"
OUT_PAGE = ROOT / "10.14.3-sop-battery-program.html"
...
<p>Commanding</p>
<hr>
<p><strong>CMDP Reference:</strong> 10(14)-3</p>
"""
...
<head>
    ...
    <title>BATTERY MAINTENANCE STANDARD OPERATING PROCEDURE - CMDP Compliance</title>
...
def strip_banner_and_cover(lines: list[str]) -> list[str]:
    ...
    while i < len(lines) and not lines[i].strip():
        i += 1
    if i < len(lines) and lines[i].startswith("# BATTERY"):
        i += 1
    while i < len(lines) and not lines[i].strip():
        i += 1
...
def main() -> None:
    ...
    header_html = f"""<hr>
<h1>BATTERY MAINTENANCE STANDARD OPERATING PROCEDURE</h1>
